/********
 文件名：FUKalmanBoxTracker_ServiceArea.cpp
 作者：Huahuan
 描述：tracker对象
 版本：v1.0
 日期：2024.01.10
...
FUKalmanBoxTracker_ServiceArea::FUKalmanBoxTracker_ServiceArea(xt::xarray<float>    bbox,
                                                               ...
                                                               bool                 from_radar)
{
    ...
    if (int(bbox(7)) == 0 || int(bbox(7)) == 2 || int(bbox(7)) == 5 || int(bbox(7)) == 6 || int(bbox(7)) == 7 || int(bbox(7)) == 8)
    {
        ...
        _kf = FUKalmanFilter_ServiceArea(6, 2, 1);
        xt::xarray<float> bbox_init = xt::view(bbox, xt::range(0, 4));
        xt::view(_kf._x, xt::range(0, 4)) = bbox_init.reshape({bbox_init.size(), 1});
    }
    else
    {
        use_acc_model = 0; // 匀速模型
        _kf._model_type = 0;
        _kf = FUKalmanFilter_ServiceArea(6, 2, 0);
        xt::xarray<float> bbox_init = xt::view(bbox, xt::range(0, 4));
        xt::view(_kf._x, xt::range(0, 4)) = bbox_init.reshape({bbox_init.size(), 1});
    }

    // _kf._x = xt::view(bbox,xt::range(0,4));
    _bbox = bbox; // 一维向量
    camera_only_update_xy = false;
    get_camera_fill_blind = false;
    // state.push_back(bbox);
    high_speed = false;
    low_speed = true;
    ...
    */
    ...
    _type_fusion = TypeFusion_ServiceArea(bbox);
    use_predict = false;

    lidar_updated = 0;
    camera_updated = 0;
    radar_updated = 0;
    lidar_continous_updated = 0;
...
FUKalmanBoxTracker_ServiceArea::FUKalmanBoxTracker_ServiceArea()
{

}


void FUKalmanBoxTracker_ServiceArea::updatewithlidar(xt::xarray<float>  &bbox, 
                                                     unsigned long      timestamp)
{
    if (lidar_updated == 1)
        lidar_continous_updated = 1;
    else
        lidar_continous_updated = 0;
...
void FUKalmanBoxTracker_ServiceArea::updatewithcamera(xt::xarray<float>  &bbox, 
                                                      unsigned long      timestamp)
{
...
void FUKalmanBoxTracker_ServiceArea::predict(float dt)
{

    update_pixel_location = {10000, 10000, 10000, 10000};
    use_predict = true;

    if(_type_fusion.fusion_label == 0 || _type_fusion.fusion_label == 2 ||
       _type_fusion.fusion_label == 5 || _type_fusion.fusion_label == 6 ||
       _type_fusion.fusion_label == 7 || _type_fusion.fusion_label == 8)
    {
        use_acc_model = 1;
        _kf._model_type = 1;
    }
...
void FUKalmanBoxTracker_ServiceArea::update(unsigned long timestamp, float dt)
{
    if (lidar_updated == 1 || camera_updated == 1 || radar_updated == 1)
    {
        ...
        else if (camera_updated == 1)
        {
            /*
            TODO:
            if (self.type_fusion.fusion_label in self.camera_create_trackers) or self.camera_fill_blinds():

                if self.camera_only_update_xy:
                    self.fusion_state[:2] = self.last_camera_state[:2]
                else:
                    self.fusion_state = self.last_camera_state
                if self.radar_updated:
                    self.fusion_state[0] = self.last_radar_state[0]
                    self.fusion_state[8] = self.last_radar_state[8]
            else:
        ...
        /*
        TODO:
        smooth_xy = self.smooth_xy(dt)
          # t0 = time.time()
          self.kf.update(smooth_xy, reduce, 1)
        */
...
state_output_ServiceArea FUKalmanBoxTracker_ServiceArea::get_state()
{

    state_output_ServiceArea state_result;
    xt::xarray<float> output_x = xt::view(_kf._x, xt::range(0, 4));

    state_result.output_x = output_x.reshape({1, 4});
    state_result.bbox = _bbox;
    state_result.speed = speed;
    return state_result;
}


void FUKalmanBoxTracker_ServiceArea::parse_json(nlohmann::json *fusion_parameter)
{
    // 解析参数文件
...
void FUKalmanBoxTracker_ServiceArea::smooth_xy(float dt)
{
    // dt = 0.1;
...
void FUKalmanBoxTracker_ServiceArea::update_wlh()
{   
    float alpha;
    if (hits > 50){   
        alpha = 1.0 / 50.0;
...
// 拷贝构造函数
FUKalmanBoxTracker_ServiceArea::FUKalmanBoxTracker_ServiceArea(const FUKalmanBoxTracker_ServiceArea &rhs)
{
    ...
    lidar_continous_updated = rhs.lidar_continous_updated;
...
void FUKalmanBoxTracker_ServiceArea::update_kalman_coefficient(const float& temp_x,
                                                               const float& temp_y,
                                                               ...
                                                               const float& dt)
{
    if (last_speed != -1000)
    {
        float temp_rad_angle = fmod(-1 * temp_angle - 90, 360);
        float rad_angle = temp_rad_angle * PI / 180;
        float last_temp_rad_angle = fmod(-1 * last_angle - 90, 360);
        float last_rad_angle = last_temp_rad_angle * PI / 180;
        _kf._x(0, 0) = temp_x;
        _kf._x(1, 0)= temp_y;
        _kf._x(2, 0) = temp_speed * cos(rad_angle);
        _kf._x(3, 0) = temp_speed * sin(rad_angle);
        float last_vx = last_speed * cos(last_rad_angle);
        float last_vy = last_speed * sin(last_rad_angle);
        if (use_acc_model == 1 && dt != 0)
        {   if (time_since_update > 0)
            {
                _kf._x(4, 0) = (_kf._x(2, 0) - last_vx) / (dt / time_since_update);
                _kf._x(5, 0) = (_kf._x(3, 0) - last_vy) / (dt / time_since_update);
            }
            else
            {
                _kf._x(4, 0) = (_kf._x(2, 0) - last_vx) / dt;
                _kf._x(5, 0) = (_kf._x(3, 0) - last_vx) / dt;
            }
        }
    }
}
