...
#ifndef FUSION_ALG_BOX_OPS_TUNNEL_H
#define FUSION_ALG_BOX_OPS_TUNNEL_H
...
namespace fusion_utils{
    float lidar_class_transform(float lidar_class);
    float video_class_transform(float video_class);
    float judge_class(float &video_class, float &lidar_class, float &video_confidience, float &lidar_confidience);
    float find_most_times_ele(const std::vector<float>& vecElesParent);
}
