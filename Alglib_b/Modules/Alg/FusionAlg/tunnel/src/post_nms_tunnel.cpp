...
# include "post_nms_tunnel.h"


void post_nms_tunnel::postNms(xt::xarray<float> &tlwh_boxes,
    ...
    remove_penple_above_bicycle_motor(tlwh_boxes, scores, class_idxs);


}
void post_nms_tunnel::remove_big_box_in_the_images(xt::xarray<float> &tlwh_boxes,
                                                    xt::xarray<float> &scores,
                                                    xt::xarray<float> &class_idxs)
{
...
void post_nms_tunnel::remove_people_in_the_vehicle(xt::xarray<float> &tlwh_boxes,
                                                    xt::xarray<float> &scores,
                                                    xt::xarray<float> &class_idxs) {

    //std::cout<<class_idxs<<std::endl;
    std::vector<int> indices(int(scores.shape(0)), 1);//box指示索引默认为1 车中人对应索引为0
    auto not_people_boxes_index = xt::where(class_idxs > 4 || class_idxs < 4)[0];//不为人的box索引
...
void post_nms_tunnel::remove_penple_above_bicycle_motor(xt::xarray<float> &tlwh_boxes,
                                                         xt::xarray<float> &scores,
                                                         xt::xarray<float> &class_idxs) {
