...
// #include <Eigen/Dense>
#include "FUKalmanFilter_tunnel.h"

#include <xtensor/xadapt.hpp>
#include <xtensor-blas/xlinalg.hpp>
//
xt::xarray<float> helpers_tunnel::reshape_z(xt::xarray<float> z, int dim_z, int ndim) {
    z = xt::atleast_2d(z);
    if (z.shape(1) == dim_z)
        z = xt::transpose(z);
...
//
FUKalmanFilter_tunnel::FUKalmanFilter_tunnel(int dim_x, int dim_z, int dim_u):
                            _dim_x(dim_x),
                            _dim_z(dim_z),
                            _dim_u(dim_u){
    assert(dim_x >= 1);
...
//
FUKalmanFilter_tunnel::FUKalmanFilter_tunnel() {}

FUKalmanFilter_tunnel::~FUKalmanFilter_tunnel() {}
//
void FUKalmanFilter_tunnel::predict(xt::xarray<float> u, xt::xarray<float> B, xt::xarray<float> F, xt::xarray<float> Q) {
    /**
...
//    std::cout<<"FFF"<<F<<std::endl;
    if (!B.size())
        B = _B;
//    std::cout<<"_dim_x "<<_dim_x<<std::endl;
//    std::cout<<"F.shape(1)"<<F.shape(1)<<std::endl;
...
//    std::cout<<_x<<"\n";
//    std::cout<<F<<"\n";
//    std::cout<<"F "<<F.shape(1)<<" B "<<B.shape(1)<<" _x "<<_x.shape(0)<<" u "<<u.shape(0)<<" _P "<<_P.shape(0)<<std::endl;
//    if (!B.size() || u.size())
//        _x = xt::linalg::dot(F, _x);
//    else
//    std::cout<<"F "<<F.shape(0)<<" "<<F.shape(1)<<" _x "<<_x.shape(0)<<" "<<_x.shape(1)<<std::endl;
//    std::cout<<"_F "<<_F.shape(0)<<" "<<_F.shape(1)<<std::endl;
    _x = xt::linalg::dot(F, _x);
...
void FUKalmanFilter_tunnel::update(xt::xarray<float> &z, xt::xarray<float> R, xt::xarray<float> H) {
    _log_likelihood = 0;
    _likelihood = 0;
    _mahalanobis = 0;
    ...
    z = helpers_tunnel::reshape_z(z, _dim_z, z_shape.size());
//    std::cout<<"H:"<<H.shape(1)<<"_x:"<<_x.shape(0)<<"_P:"<<_P.shape(1)<<"_SI"<<_SI.shape(0)<<std::endl;
//    std::cout<<"_K:"<<_K.shape(1)<<"_y:"<<_y.shape(0)<<std::endl;
//    std::cout<<z<<"\n";
    if (!R.size())
        R = _R;
    if (!H.size())
