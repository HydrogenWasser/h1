...
//
xt::xarray<float> helpers_crossing::reshape_z(xt::xarray<float> z, int dim_z, int ndim) {
    z = xt::atleast_2d(z);
    if (z.shape(1) == dim_z)
        z = xt::transpose(z);
...
//
FUKalmanFilter_crossing::FUKalmanFilter_crossing(int dim_x, int dim_z, int dim_u):
                            _dim_x(dim_x),
                            _dim_z(dim_z),
                            _dim_u(dim_u){
    assert(dim_x >= 1);
...
//
FUKalmanFilter_crossing::FUKalmanFilter_crossing() {}

FUKalmanFilter_crossing::~FUKalmanFilter_crossing() {}
//
void FUKalmanFilter_crossing::predict(xt::xarray<float> u, xt::xarray<float> B, xt::xarray<float> F, xt::xarray<float> Q) {
    /**
...
void FUKalmanFilter_crossing::update(xt::xarray<float> &z, xt::xarray<float> R, xt::xarray<float> H) {
    _log_likelihood = 0;
    _likelihood = 0;
    _mahalanobis = 0;
    ...
    z = helpers_crossing::reshape_z(z, _dim_z, z_shape.size());
    if (!R.size())
        R = _R;
    if (!H.size())
        H = _H;
