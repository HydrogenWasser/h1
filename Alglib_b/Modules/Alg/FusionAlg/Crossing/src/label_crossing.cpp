...
int label_crossing::lidar_class_transform(int lidar_id) {
    int output_class;
    if (lidar_id == 0)  // car
        output_class = 0;
...
int label_crossing::video_class_transform(int video_id) {
    int output_class;
...
// 相机转换至雷达  不同类别的取值
std::string label_crossing::class2size(int class_id) {
...
bool label_crossing::judge_large_target(int target_class)
{
    if (target_class == 2 || target_class == 3 || target_class == 7 || target_class == 8 || target_class == 10 || target_class == 11)
        return true;
    else
        return false;
}

void label_crossing::lidar_class_judge( xt::xarray<float> &data_temp,float &lidar_class)
{
...
int label_crossing::fusion_class(int lidar_id, int video_id) {
    int final_class;
    ...
    // else if (video_id == 8 || video_id == 9 || video_id == 10 || video_id == 11 || video_id == 12|| video_id == 13|| video_id == 14)
    // {
    //     //motorbicycle
    //     if (lidar_id == 3)
    ...
    //         final_class = video_id;
    // }
    // else if (video_id == 4 || video_id == 5 || video_id == 6 || video_id == 16) {
    //     //bus & truck
    //     final_class = video_id;
    // }
    // else if (video_id == 0) {
    ...
    //         final_class = video_id;
    // }
    // else if (video_id == 2 || video_id == 3 || video_id == 7 || video_id == 8 || video_id == 10 || video_id == 11) {
    //     //bus & truck
    //     final_class = video_id;
    // }
    // else if (video_id == 4) {
