...
DistanceAssociate::DistanceAssociate(cal_dis_result* _cal_dis_result, nlohmann::json *parameter)
{
...
void DistanceAssociate::pre_process_associate_data(xt::xarray<float> *_dets, xt::xarray<float> *_trks, int flag, nlohmann::json *parameter)
{   
    m_flag = flag;
    ...
    if (flag == 1){  // 雷达距离匹配
        ...
        // m_detections = xt::zeros<float>({int(dets.shape(0)), 3}); // x, y,  theta
        // m_trackers = xt::zeros<float>({int(trks.shape(0)), 3});   // x, y,  theta

        // std::vector<int> cols1 = {0, 1, 4};
        // std::vector<int> cols2 = {0, 1, 6};
        // for(int i = 0; i<cols1.size(); i++){
        //     xt::view(m_detections, xt::all(), i) = xt::view(dets, xt::all(), cols1[i]);
        //     xt::view(m_trackers, xt::all(), i) = xt::view(trks, xt::all(), cols2[i]);
        // }
        m_detections = *_dets;
        m_trackers = *_trks;
    }
...
void DistanceAssociate::execute_match(xt::xarray<float> _cost_matrix, fusion_match_out *associate_out, nlohmann::json *parameter)
{
...
void DistanceAssociate::post_process_associate_data(fusion_match_out *associate_out, xt::xarray<float> *_dets, int flag, nlohmann::json *parameter)
{
        ...
        if (flag == 1){
            if (int(m_trackers(trk_index, 8)) == 4){
                if (m_cal_dis_result->head_dis(det_index, trk_index) > float((*parameter)["fusion_param"]["lidar_head_disThreshold_person"])
                    ...
                    || m_cal_dis_result->square_dis(det_index, trk_index) > FunctionHub::neighbor_search(m_trackers, trk_index, float((*parameter)["fusion_param"]["search_radius_person"]), parameter)[quadrant(det_index, trk_index)]){
                    unmatched_det_index.push_back(det_index);
                    unmatched_trk_index.push_back(trk_index);
                    // cout<<"zb11111111111111111111"<<FunctionHub::neighbor_search(m_trackers, trk_index, float((*parameter)["fusion_param"]["search_radius_person"]), parameter)[quadrant(det_index, trk_index)]<<endl;
                }
                else{
                ...
                if (m_cal_dis_result->head_dis(det_index, trk_index) > (m_cal_dis_result->head_dis_thres_matrix(det_index, trk_index)
                    ...
                    || (int(m_trackers(trk_index, 8)) == 1 && m_cal_dis_result->square_dis(det_index, trk_index) > FunctionHub::neighbor_search(m_trackers, trk_index, float((*parameter)["fusion_param"]["search_radius_bicycle"]), parameter)[quadrant(det_index, trk_index)])){
                    unmatched_det_index.push_back(det_index);
                    unmatched_trk_index.push_back(trk_index);
                    // cout<<"zb222222222222222222222"<<FunctionHub::neighbor_search(m_trackers, trk_index, float((*parameter)["fusion_param"]["search_radius_bicycle"]), parameter)[quadrant(det_index, trk_index)]<<endl;
                }
                else{
                    matched_det_index.push_back(det_index);
                    matched_trk_index.push_back(trk_index);
...
void DistanceAssociate::post_process_associate_data_third(fusion_match_out *associate_out, xt::xarray<float> *_dets, int flag, nlohmann::json *parameter, std::vector<int> *vaild_elder, std::vector<int> *vaild_younger)
{
...
void DistanceAssociate::parse_json(nlohmann::json *parameter)
{
