...
Fusion_Algorithm_Intersection::Fusion_Algorithm_Intersection(TSelfFusionAlgParam *m_stAlgParam)
{

    m_FusionTrackAlg = new FusionTrackAlg(&(m_stAlgParam->m_fusion_parameter));
}
...
Fusion_Res Fusion_Algorithm_Intersection::RUN(std::vector<xt::xarray<float>> &video, xt::xarray<float> &data, int cam_num)
{

    // std::cout << "liluo------------------> RUN 111" << std::endl;
    m_FusionTrackAlg->predict(m_pc_timestamp);
    // std::cout << "liluo------------------> RUN  112" << std::endl;
    m_FusionTrackAlg->set_lidar_data(&data, m_pc_timestamp);
    m_FusionTrackAlg->set_camera_data(&video, m_camera_timestamp);
    // std::cout << "liluo------------------> RUN  113" << std::endl;

    xt::xarray<float> fusion_track_result = xt::empty<float>({0, 19});
    // xt::xarray<float> *fusion_track_result;

    m_FusionTrackAlg->process(m_pc_timestamp, &fusion_track_result);
    // std::cout << "liluo------------------> RUN  114" << std::endl;

    _output.pc_res = fusion_track_result;
    _output.video_res = video;
    // std::cout << "liluo------------------> RUN  115" << std::endl;

    return _output;
}

// Fusion_Res Fusion_Algorithm_Intersection::RUN(TPcResult *p_pPcResult, TVideoResult *p_pVideoResult)
// {
