...
*/
xt::xarray<float> box_ops::corners_nd(xt::xarray<float> &dims, float origin) {
...
*/
xt::xarray<float> box_ops::rotation_2d(xt::xarray<float> &corners, const xt::xarray<float> &angles) {
...
*       [type] : [description]
*/
xt::xarray<float> box_ops::center_to_corner_box2d(xt::xarray<float> &centers, xt::xarray<float> &dims,
                                               xt::xarray<float> &angles, float origin) {
//    std::cout<<dims<<"\n";
    xt::xarray<float> corners = box_ops::corners_nd(dims, origin);

    if (angles.size())
        corners = box_ops::rotation_2d(corners, angles);
...
*       [type] : [description]
*/
xt::xarray<float> box_ops::center_to_corner_box3d(xt::xarray<float> &centers, xt::xarray<float> &dims,
                                                  float &angles, float origin, int axis) {
    // angles = angles*M_PI / 180.0;
    xt::xarray<float> corners = box_ops::corners_nd(dims, origin);
    if (angles != float (None))
        box_ops::rotation_3d_in_axis(corners, angles, axis);
//    std::cout<<corners<<"\n";
...
xt::xarray<float> box_ops::corner_to_standup_nd(xt::xarray<float> &boxes_corner) {
...
std::pair<xt::xarray<float>, xt::xarray<float>> box_ops::rotate_nms_cc(xt::xarray<float> &dets,
    ...
    xt::xarray<float> trackers_corners = box_ops::center_to_corner_box2d(t1, t2, t3);
    xt::xarray<float> trackers_standup = box_ops::corner_to_standup_nd(trackers_corners);
//    std::cout<<trackers_standup<<"\n";
    ...
    xt::xarray<float> dets_corners = box_ops::center_to_corner_box2d(d1, d2, d3);
    xt::xarray<float> dets_standup = box_ops::corner_to_standup_nd(dets_corners);
//    std::cout<<dets_standup<<"\n";
    return box_ops::iou_jit_new(dets_standup, trackers_standup, 0.0);
}


float box_ops::cal_angle(std::vector<xt::xarray<float>> &state_list, float &thresh) {
...
std::pair<xt::xarray<float>, xt::xarray<float>> box_ops::iou_jit_new(xt::xarray<float> &boxes,
...
void box_ops::rotation_3d_in_axis(xt::xarray<float> &points, float &angles, int axis) {
...
*/
float box_ops::IOU_calc(xt::xarray<float> box1, xt::xarray<float> box2, bool wh) {
