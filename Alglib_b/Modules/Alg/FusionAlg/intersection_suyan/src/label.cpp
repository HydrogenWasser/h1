...
int label::lidar_class_transform(int lidar_id) {
    int output_class;
    if (lidar_id == 0)  // car
        output_class = 0;
...
int label::video_class_transform(int video_id) {
    int output_class;
...
// 相机转换至雷达  不同类别的取值
std::string label::class2size(int class_id) {
...
bool label::judge_large_target(int target_class)
{
...
void label::lidar_class_judge( xt::xarray<float> &data_temp,float &lidar_class)
{
...
int label::fusion_class(int lidar_id, int video_id) {
    int final_class;
