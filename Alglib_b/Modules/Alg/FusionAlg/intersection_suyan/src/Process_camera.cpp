...
Process_camera::Process_camera(nlohmann::json *parameter, std::unordered_map<int, std::pair<int, float>> *chedao, fusion_match_out *match_out, cal_dis_result *cal_dis_result, float x_limit[], float y_limit[])
{
...
void Process_camera::process_camera_data(xt::xarray<float> *_p_pCameraResult)
{
    ...
    // std::vector<xt::xarray<float>> video_for_fusion_all_channel;
    // int camera_dev_num = _p_pCameraResult->m_vecResult.size();
    // for (int i = 0; i < camera_dev_num; i++){  // 便利每一路相机
    //     int each_video_box_num = _p_pCameraResult->m_vecResult[i].m_vecBox.size();
    //     if (each_video_box_num == 0){  // 该路相机的目标框集个数为0
    //         video_for_fusion_all_channel.push_back(xt::empty<float>({0, 17}));
    //     }
    //     else{
    //         xt::xarray<float> each_video_for_fusion_box = xt::zeros<float>({each_video_box_num, 17});
    //         for (int j = 0; j < each_video_box_num; j++){  // 遍历某一路相机的所有目标框
    //             each_video_for_fusion_box(j, 0) = _p_pCameraResult->m_vecResult[i].m_vecBox[j].m_sXCoord;  // x
    //             each_video_for_fusion_box(j, 1) = _p_pCameraResult->m_vecResult[i].m_vecBox[j].m_sYCoord;  // y
    //             each_video_for_fusion_box(j, 2) = _p_pCameraResult->m_vecResult[i].m_vecBox[j].m_usWidth;  // w
    //             each_video_for_fusion_box(j, 3) = _p_pCameraResult->m_vecResult[i].m_vecBox[j].m_usLength;   // l
    //             each_video_for_fusion_box(j, 4) = _p_pCameraResult->m_vecResult[i].m_vecBox[j].m_sTheta;   // theta
    //             each_video_for_fusion_box(j, 5) = _p_pCameraResult->m_vecResult[i].m_vecBox[j].m_sZCoord;    // z
    //             each_video_for_fusion_box(j, 6) = _p_pCameraResult->m_vecResult[i].m_vecBox[j].m_usHeight;     // h
    //             each_video_for_fusion_box(j, 7) = 0;  // label
    //             each_video_for_fusion_box(j, 8) = _p_pCameraResult->m_vecResult[i].m_vecBox[j].m_sSpeed;
    //             each_video_for_fusion_box(j, 9) = _p_pCameraResult->m_vecResult[i].m_vecBox[j].m_usSingleId;
    //             each_video_for_fusion_box(j, 10) = _p_pCameraResult->m_vecResult[i].m_vecBox[j].m_ucConfidence;
    //             each_video_for_fusion_box(j, 11) = _p_pCameraResult->m_vecResult[i].m_vecBox[j].m_fTopLeftX;
    //             each_video_for_fusion_box(j, 12) = _p_pCameraResult->m_vecResult[i].m_vecBox[j].m_fTopLeftY;
    //             each_video_for_fusion_box(j, 13) = _p_pCameraResult->m_vecResult[i].m_vecBox[j].m_fBottomRightX;
    //             each_video_for_fusion_box(j, 14) = _p_pCameraResult->m_vecResult[i].m_vecBox[j].m_fBottomRightY;
    //             each_video_for_fusion_box(j, 15) = _p_pCameraResult->m_vecResult[i].m_vecBox[j].m_sDataSource;
    //             each_video_for_fusion_box(j, 16) = _p_pCameraResult->m_vecResult[i].m_vecBox[j].m_sChannel;

    //         }
    //         /* 根据相机反映射配置，对相机检测目标进行过滤*/
    // auto filter_box_index = xt::where(xt::col(each_video_for_fusion_box, m_camera_reflect_limit(i, 0)) > m_camera_reflect_limit(i, 2) || xt::col(each_video_for_fusion_box, m_camera_reflect_limit(i, 0)) < m_camera_reflect_limit(i, 1))[0];
    //         each_video_for_fusion_box = xt::view(each_video_for_fusion_box, xt::drop(filter_box_index));

    //         auto col = xt::view(each_video_for_fusion_box, xt::all(), 10);
    //         col = xt::where(col >= 1.0, 0.98, col);

    //         auto filter_box_x_index = xt::where(xt::col(each_video_for_fusion_box, 0) <= -1000)[0];
    //         each_video_for_fusion_box = xt::view(each_video_for_fusion_box, xt::drop(filter_box_x_index));

    //         video_for_fusion_all_channel.push_back(each_video_for_fusion_box);
    //     }
    // }
    ...
    //         xt::view(camera_box, xt::range(last_box_num, last_box_num + each_box_num), xt::all()) = xt::view(*it, xt::all(), xt::all());
    ...
    // FunctionHub::result_nms(camera_box, delete_index, float((*m_fusion_parameter)["fusion_param"]["camera_nms_min_threshold"]), float((*m_fusion_parameter)["fusion_param"]["camera_nms_max_threshold"]), true);




}
void Process_camera::excute_match_camera_stage(std::vector<FUKalmanBoxTracker> &_trackers, IouAssociate *iou_associate, DistanceAssociate *dis_associate)
{
    ...
    if (int(trks_for_second.shape(0)) != 0 && int(dets_for_second.shape(0)) != 0)
    {
        ...
        dis_associate->pre_process_associate_data(&dets_for_second, &trks_for_second, 0, m_fusion_parameter);
        ...
        dis_associate->post_process_associate_data(m_associate_out, &dets_for_second, 0, m_fusion_parameter);
    }
...
void Process_camera::load_csv(std::string _file_path)
/*
