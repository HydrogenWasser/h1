...
FUKalmanBoxTracker::FUKalmanBoxTracker(xt::xarray<float> bbox, unsigned long time_stamp, nlohmann::json *fusion_parameter, bool from_lidar, bool from_camera, bool from_radar)
{
    ...
    */
    ...
    _type_fusion = TypeFusion(bbox);
    use_predict = false;

    lidar_updated = 0;
    camera_updated = 0;
    radar_updated = 0;
    lidar_last_updated = 1;
    time_since_create = 0;
...
{
    /*
    if self.lidar_updated == 1:

        self.lidar_continous_updated = 1
    else:
        self.lidar_continous_updated = 0
    self.lidar_updated = 1

    self.last_lidar_state = bbox
    self.lidar_timestamp = timestamp

    # #####*****  根据lidar的类别和置信度进行类别的更新(后续需要考虑不同传感器的置信度)  *****#####
    self.type_fusion.updateType(bbox,src=2)
    */
    if (time_since_create < int((*m_fusion_parameter)["fusion_param"]["sort_min_hits"])){
        if (lidar_last_updated == 1 && lidar_continous_updated == 1)
        {
            lidar_continous_updated = 1;
        }
        else
...
void FUKalmanBoxTracker::updatewithcamera(xt::xarray<float> &bbox, unsigned long timestamp)
{
    ...
    // _type_fusion.fusion_label = bbox(7);  //zb, debug
...
void FUKalmanBoxTracker::predict(float dt)
{
    ...
    if(_type_fusion.fusion_label == 0 || _type_fusion.fusion_label == 2 || _type_fusion.fusion_label == 5 || _type_fusion.fusion_label == 6 || _type_fusion.fusion_label == 7 || _type_fusion.fusion_label == 8){
        use_acc_model = 1;
        _kf._model_type = 1;
    }else{
        use_acc_model = 0;
...
{
    if (time_since_create != 0){
        lidar_last_updated = lidar_updated;
    }
...
state_output FUKalmanBoxTracker::get_state()
{

    state_output state_result;
...
FUKalmanBoxTracker::FUKalmanBoxTracker(const FUKalmanBoxTracker &rhs)
{
    ...
    time_since_create = rhs.time_since_create;
    lidar_last_updated = rhs.lidar_last_updated;
}
