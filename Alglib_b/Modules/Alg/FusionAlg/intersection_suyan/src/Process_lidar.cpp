...
Process_lidar::Process_lidar(nlohmann::json *parameter, std::unordered_map<int, std::pair<int, float>> *chedao, fusion_match_out *match_out, cal_dis_result *cal_dis_result, float x_limit[], float y_limit[])
{
...
void Process_lidar::process_lidar_data(xt::xarray<float> *_p_pPcResult)
{
    ...
    // lidar_box = xt::zeros<float>({int(_p_pPcResult->m_vecBox.size()), 17});
    // int label;
    // for (int i = 0; i < int(_p_pPcResult->m_vecBox.size()); i++)
    // {
    //     lidar_box(i, 0) = float(_p_pPcResult->m_vecBox[i].m_sXCoord) / 100;                  // x
    //     lidar_box(i, 1) = float(_p_pPcResult->m_vecBox[i].m_sYCoord) /100;                  // y
    //     lidar_box(i, 2) = float(_p_pPcResult->m_vecBox[i].m_usWidth) /100;                  // w
    //     lidar_box(i, 3) = float(_p_pPcResult->m_vecBox[i].m_usLength) /100;                 // l
    //     lidar_box(i, 4) = (float(_p_pPcResult->m_vecBox[i].m_usCourseAngle) /100) * 180 / PI; // theta
    //     lidar_box(i, 5) = float(_p_pPcResult->m_vecBox[i].m_sZCoord)/100;                  // z
    //     lidar_box(i, 6) = float(_p_pPcResult->m_vecBox[i].m_usHeight) /100;                 // h

    //     label = FunctionHub::pcClass2label(_p_pPcResult->m_vecBox[i].m_strClass, m_fusion_parameter);
    ...
    //     lidar_box(i, 8) = float(_p_pPcResult->m_vecBox[i].m_usSpeed);       // speed
    //     lidar_box(i, 9) = _p_pPcResult->m_vecBox[i].m_usSingleId;    // id
    //     lidar_box(i, 10) = float(_p_pPcResult->m_vecBox[i].m_ucConfidence)/100; // score
...
void Process_lidar::excute_match_lidar_stage(std::vector<FUKalmanBoxTracker> &_trackers, IouAssociate *iou_associate, DistanceAssociate *dis_associate)
{
    ...
    if(int(trks_for_second.shape(0)) != 0 && int(dets_for_second.shape(0)) != 0){
        ...
        dis_associate->pre_process_associate_data(&dets_for_second, &trks_for_second, 1, m_fusion_parameter);
...
void Process_lidar::excute_match_lidar_stage_3rd_match(std::vector<FUKalmanBoxTracker> &_trackers, IouAssociate *iou_associate, DistanceAssociate *dis_associate)
{
    track_box = xt::empty<float>({0, 10});
