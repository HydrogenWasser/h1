...
struct fusion_match_out{
    ...
    // std::vector<FUKalmanBoxTracker>* ptr_wait_update_trks;  // 待进行类别更新的轨迹的指针
...
public:
    ...
    // 关联之前数据预处理
    virtual void pre_process_associate_data(xt::xarray<float>* _dets, xt::xarray<float>* _trks, int flag, nlohmann::json *parameter)=0;  

    // // 计算代价矩阵
    // virtual xt::xarray<float> cal_cost_matrix(){};
    virtual void cal_cost_matrix(xt::xarray<float> &cost_matrix) = 0;

    // 匈牙利匹配
    virtual void execute_match(xt::xarray<float> _cost_matrix, fusion_match_out* associate_out, nlohmann::json *parameter)=0;

    // 后处理
    virtual void post_process_associate_data(fusion_match_out *associate_out, xt::xarray<float>* _dets, int flag, nlohmann::json *parameter)=0;
