...
public:
    ...
    std::vector<int64_t> get_num_upsample_filters();
...
private:
    ...
    /** ======================kitti: conv box & conv cls & conv dir ====================*/
//    torch::nn::Conv2d _con_cls = torch::nn::Conv2d(torch::nn::Conv2dOptions(256, 56, 1).stride(1).padding(0).groups(1).bias(
//            true));
...
//            true));
    ...
    /** ======================32line: conv box & conv cls & conv dir ====================*/
    torch::nn::Conv2d _con_box = torch::nn::Conv2d(torch::nn::Conv2dOptions(256, 98, 1).stride(1).padding(0).groups(1).bias(
            true));
    torch::nn::Conv2d _conv_dir_cls = torch::nn::Conv2d(torch::nn::Conv2dOptions(256, 28, 1).stride(1).padding(0).groups(1).bias(
            true));
    torch::nn::Conv2d _con_cls = torch::nn::Conv2d(torch::nn::Conv2dOptions(256, 98, 1).stride(1).padding(0).groups(1).bias(
            true));
...
class DefaultHead{
public:
    DefaultHead(int64_t num_filters,
                int num_class,
                int num_anchor_per_loc,
                int64_t box_code_size,
                int64_t num_direction_bins,
                bool use_direction_classifier,
                bool encode_background_as_zeros,
    ...
    std::map<std::string, torch::Tensor> forward(torch::Tensor &x);
    void load_weight(std::string head_type, std::map<std::string, torch::Tensor> &weights);
...
private:
    ...
    bool _use_direction_classifier;
    int64_t _box_code_size;
    torch::nn::Sequential _conv_cls;
    torch::nn::Sequential _conv_box;
    torch::nn::Sequential _conv_dir_cls;
    std::string _head_type;
    std::map<std::string, torch::Tensor> _weight_head;
};
