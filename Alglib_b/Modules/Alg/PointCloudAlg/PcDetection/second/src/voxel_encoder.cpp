...
void VoxelGeneratorV2::points_to_voxel(xt::xarray<float> &points, int64_t max_voxels,
    ...
    // std::cout<<"============= _max_num_points : "<<_max_num_points<<"\n";
...
//    std::cout<< grid_size<<std::endl;
    ...
    // std::cout<<(int) grid_size[2]<<" " << (int)grid_size[1] << " " << (int) grid_size[0]<< " \n";
...
torch::Tensor SimpleVoxelRadius::forward_simple(xt::xarray<float> features, xt::xarray<int> num_voxels) {
    auto t1 = std::chrono::steady_clock::now();
    ...
    auto t2 = std::chrono::steady_clock::now();
    auto points_mean = features_t.slice(2, 0, _num_input_features).sum(1, false) /
            num_voxels_t.type_as(features_t).view({-1, 1});
    points_mean = points_mean.contiguous();
    auto t3 = std::chrono::steady_clock::now();

    double vfe_tranfer = std::chrono::duration<double, std::milli>(t2 - t1).count();
    double vfe_calcuate = std::chrono::duration<double, std::milli>(t3 - t2).count();
    printf("vfe_tranfer cost : %.4f \n", vfe_tranfer);
    printf("vfe_calcaute cost : %.4f \n", vfe_calcuate);
    std::cout<<_num_input_features<<std::endl;
    return points_mean;
...
torch::Tensor SimpleVoxelRadius::forward_xt(xt::xarray<float> features, xt::xarray<int> num_voxels) {
    ...
    torch::Tensor num_voxels_t = torch::from_blob(num_voxels.data(), {num_shape(0), }, torch::kInt32).to(torch::kCUDA);
    auto t2 = std::chrono::steady_clock::now();
//    std::cout<<features_t.sizes()<<std::endl;
//    std::cout<<
    auto points_mean =
